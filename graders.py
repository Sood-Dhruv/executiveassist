...
def grade_action(
    ...
    if grader is None:
        return 0.001, f"Unknown task '{task_id}'.", True

    try:
        return grader(task, action, step, history)
    except Exception as e:
        return 0.001, f"Grader error: {str(e)}", False
...
def grade_schedule_meeting(task, action, step, history):
    ...
    if action.get("type") != "schedule":
        return 0.001, "Expected action type 'schedule'.", False
    ...
    feedback = "\n".join(notes) + f"\n\nScore: {score:.2f}"
    return _clamp(score), feedback, done
...
def grade_inbox_triage(task, action, step, history):
    if action.get("type") != "triage":
        return 0.001, "Expected action type 'triage'.", False
...
def grade_draft_reply(task, action, step, history):
    if action.get("type") != "reply":
        return 0.001, "Expected action type 'reply'.", False
    ...
    concrete_date_patterns = [
        r"july\s*1[5-9]", r"jul\s*1[5-9]",
        r"17th|16th|18th|19th|15th",
        r"\bby\s+(end|eod|monday|tuesday|wednesday|thursday|friday|next)",
        r"within\s+\d+\s+(day|hour|business)",
        r"deliver(y|ed|ing)?\s+(by|on|before)",
    ...
    has_concrete = any(re.search(p, body) for p in concrete_date_patterns)
    if has_concrete:
...
def grade_multi_party_schedule(task, action, step, history):
    ...
    if not schedule_actions:
        return 0.001, "No schedule action found.", False
...
def grade_meeting_notes_extraction(task, action, step, history):
    ...
    if not extract_actions:
        if action.get("type") == "reply":
            ...
            return 0.001, "\n".join(notes), False
        return 0.001, "Expected action type 'extract'.", False
...
def grade_full_day_plan(task, action, step, history):
    ...
    if not plan_actions:
        return 0.001, "Expected action type 'plan'.", False
